    ...
    for medium in medium_types:
        ...
        if posts and isinstance(posts, dict):
            for post_id, post_data in posts.items():
                ...
                if record_date:
                    try:
                        ts = record_date / 1000 if record_date > 10000000000 else record_date
                        dt = datetime.fromtimestamp(ts)
                        print(f"  recordCreationDate parsed: {dt.strftime('%Y-%m-%d %H:%M:%S')}")
                    except:
                        print(f"  recordCreationDate parsing failed")
                ...
                if timestamp:
                    try:
                        ...
                        print(f"  timestamp parsing failed")
