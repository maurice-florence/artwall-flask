...
class ParserService:
    ...
    def _process_note(note_elem: ET.Element, author_id: str):
        ...
        created_timestamp = ParserService._parse_enex_date(created_elem.text) if created_elem is not None and created_elem.text else time.time()
        
        updated_elem = note_elem.find('updated')
        updated_timestamp = ParserService._parse_enex_date(updated_elem.text) if updated_elem is not None and updated_elem.text else created_timestamp
        ...
        content_clean = ParserService._sanitize_enml(content_raw if content_raw else "", resources)
