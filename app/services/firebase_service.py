...
def init_firebase(app):
    ...
    if not firebase_admin._apps:
        ...
        if not cred_path:
            app.logger.warning("FIREBASE_CREDENTIALS_PATH not set. Firebase will not be initialized.")
            return
...
def get_paginated_posts(limit: int = 20, end_at: Optional[float] = None) -> Tuple[List[Dict], Optional[float]]:
    """
    ...
    try:
            ...
            if result and isinstance(result, dict):
                for post_id, post_data in result.items():
                    if isinstance(post_data, dict):
                        ...
                        if 'timestamp' not in post_data and 'recordCreationDate' in post_data:
        ...
        posts = all_posts[start_index:start_index + limit]
