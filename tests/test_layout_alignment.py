...
class TestLayoutAlignment:
    ...
    def test_body_padding(self):
        ...
        if "horizontal" in padding:
            assert (
                "75px" in padding["horizontal"]
            ), f"Body horizontal padding should be 75px, got {padding['horizontal']}"
    ...
    def test_navbar_wrapper_padding(self):
        ...
        masonry_padding = self.extract_css_value(
            masonry_css, ".masonry-container", "padding"
        )
        ...
        if navbar_pad and "horizontal" in navbar_pad:
            assert (
                "24px" in navbar_pad["horizontal"]
            ), f"Navbar wrapper horizontal padding should be 24px, got {navbar_pad['horizontal']}"

        if masonry_pad and "horizontal" in masonry_pad:
            assert (
                "24px" in masonry_pad["horizontal"]
            ), f"Masonry container horizontal padding should be 24px, got {masonry_pad['horizontal']}"
    ...
    def test_total_horizontal_spacing(self):
        ...
        assert total_spacing == expected_total, (
            f"Total horizontal spacing should be {expected_total}px "
            f"(75px body + 24px wrapper), got {total_spacing}px"
        )

        print(
            f"✓ Total horizontal spacing: {body_side}px (body) + {navbar_side}px (wrapper) = {total_spacing}px"
        )
    ...
    def test_card_aspect_ratio(self):
        ...
        aspect_ratio_pattern = (
            r"\.grid-sizer,\s*\.grid-item\s*\{[^}]*aspect-ratio:\s*([^;]+);"
        )
    ...
    def test_year_separator_alignment(self):
        ...
        separator_pattern = (
            r"\.grid-item\.year-separator\s*\{[^}]*aspect-ratio:\s*([^;]+);"
        )
    ...
    def test_card_back_primary_color(self):
        ...
        assert (
            "var(--theme-primary" in background
        ...
        assert (
            "#ffffff" in title_color.lower() or "#fff" in title_color.lower()
    ...
    def test_responsive_mobile_padding(self):
        ...
        if match:
            ...
            print(
                "⚠ Mobile navbar-wrapper padding not explicitly set (inherits from default)"
            )
