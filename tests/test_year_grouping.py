...
def test_year_grouping():
    ...
    for year, year_posts in grouped:
        ...
        for post in year_posts:
            print(
                f"  - {post['title']} ({post['year']}-{post.get('month', 1):02d}-{post.get('day', 1):02d}) [{post['medium']}]"  # noqa: E501
            )
    ...
    assert (
        len(grouped[0][1]) == 2
    ), f"Expected 2 posts in 2024, got {len(grouped[0][1])}"
    assert (
        grouped[1][0] == 2023
    ), f"Expected second year to be 2023, got {grouped[1][0]}"
    assert (
        len(grouped[1][1]) == 2
    ), f"Expected 2 posts in 2023, got {len(grouped[1][1])}"
